    ...
    : VertexBuffer()
{
    ...
    for (uint32_t i = 0; i < numCharacters; i++)
    {
        ...
        vertices[vertexOffset+0] = { Vector3(i*.5f+0.0f, 0.0f, 0.0f), Vector3::Backward, Vector2(0.0f, 0.0f) };
        vertices[vertexOffset+1] = { Vector3(i*.5f+.5f, 0.0f, 0.0f), Vector3::Backward, Vector2(uvStride, 0.0f) };
        vertices[vertexOffset+2] = { Vector3(i*.5f+0.0f, -1.0f, 0.0f), Vector3::Backward, Vector2(0.0f, 1.0f) };
        vertices[vertexOffset+3] = { Vector3(i*.5f+.5f, -1.0f, 0.0f), Vector3::Backward, Vector2(uvStride, 1.0f) };
    }
